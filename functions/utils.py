...
async def add_task(app, message: Message):
    try:
        ...
        if new_file:
            ...
            await handle_upload(app, new_file, message, msg, random, filepath)
...
async def handle_upload(app, new_file, message, msg, random, original_file_path):
    user_id = str(message.from_user.id)
    base_path = os.path.join(DOWNLOAD_DIR, user_id, random)
    ...
    if os.path.exists(thumb_image_path):
        ...
        thumb = get_thumbnail(new_file, base_path, duration / 4)
    ...
    try:
        ...
        try:
            if os.path.exists(new_file):
                os.remove(new_file)
                print(f"Converted file removed: {new_file}")
            if os.path.exists(original_file_path):
                os.remove(original_file_path)
                print(f"Original file removed: {original_file_path}")
            if os.path.exists(base_path):
                shutil.rmtree(base_path)
                print(f"Directory removed: {base_path}")
            if thumb != thumb_image_path and os.path.exists(thumb):
                os.remove(thumb)
                print(f"Temporary thumbnail removed: {thumb}")
